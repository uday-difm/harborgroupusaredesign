...
// file: app/dashboard/add-blog/page.jsx
'use client';
...
export default function AddBlog() {
  ...
  const mountedRef = useRef(false); // guard for client render only
    ...
    const fetchUser = async () => {
      try {
        ...
        if (res.ok) {
          ...
          // set user info if needed (not used here beyond auth)
        } else if (res.status === 401 || res.status === 403) {
          router.push('/dashboard/login');
        } else {
          const err = await res.text();
          setUserError(err || 'Failed to fetch user data');
        }
      } catch (err) {
        console.error('Fetch user error:', err);
  ...
  const handleFileChange = (e) => {
    ...
    setValues((p) => ({ ...p, featureImage: file }));
  };

  // use onBlur from Jodit to update content safely
  ...
  const validateRequired = () => {
    const required = [
      'blogTitle', 'tags', 'featureImage', 'date', 'time', 'blogCategory', 'description', 'content'
    ];
    const missing = required.filter((k) => {
      if (k === 'featureImage') return !values.featureImage;
      const v = values[k];
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (values.featureImage instanceof File) {
        formData.append('featureImage', values.featureImage, values.featureImage.name);
      }
      formData.append('date', values.date);
      formData.append('time', values.time);
      formData.append('blogCategory', values.blogCategory);
      ...
      // Debug: log keys (for File show name)
      for (const entry of formData.entries()) {
        const [k, v] = entry;
        console.log(k, (v instanceof File) ? `[File:${v.name}]` : v);
      }

      const res = await fetch('/api/dashboard/addblog', {
        method: 'POST',
        body: formData,
      });

      // read response text for better debug: server might send HTML error page for 404
      ...
      if (!res.ok) {
        ...
        alert(`Submission error: ${res.status} - ${res.statusText}\nServer response: ${text}`);
        return;
      }

      // success (prefer json.message)
      const message = (json && json.message) ? json.message : text || 'Submitted successfully';
      alert(message);
      ...
      alert('Submission exception: ' + String(err));
    } finally {
      setLoading(false);
    }
  };
            ...
            <div className="p-6 space-y-8">
              ...
              <div>
                ...
                <input
                  ...
                  className="w-full rounded border border-stroke py-3 px-4 dark:border-form-strokedark dark:bg-form-input"
                />
                <p className="text-sm text-gray-500 dark:text-gray-300">If left blank, a slug will be generated from title.</p>
              </div>
                ...
                <div>
                  ...
                  >
                    ...
                    {categories.length ? categories.map((cat) => (
                      <option key={cat.id ?? cat._id ?? cat.category} value={cat.id ?? cat._id ?? cat.id}>
              ...
              <div>
                ...
                {mountedRef.current ? (
                  <JoditEditor
                    ...
                    config={editorConfig}
                    value={values.content}
                    // prefer onBlur to avoid the "Node element must be in editor" race conditions
