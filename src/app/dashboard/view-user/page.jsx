...
export default function AdminTable() {
  ...
  const fetchAdmins = async () => {
    try {
      const res = await fetch(`/api/dashboard/view-user`);
...
const confirmDelete = async () => {
  ...
  try {
    const res = await fetch(`/api/dashboard/delete-user/${userToDelete}`, {
      method: 'PUT',
  ...
  const columns = [
    ...
    { label: "Actions", key: "actions", render: (row) => (
      <div className="flex space-x-3 justify-center">
        <button onClick={() => router.push(`/dashboard/update-user/${row.id}`)}>
          <Pencil className="w-4 h-4 text-navy-500 hover:text-accent cursor-pointer transition" />
        </button>
        <button onClick={() => setUserToDelete(row.id)}>
