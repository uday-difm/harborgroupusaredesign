...
export async function POST(req) {
  try {
    ...
    const [result] = await pool.execute(
      `INSERT INTO majormedical (name, email, message) 
       VALUES (?, ?, ?)`,
      [name, email, message]
    );
    ...
    const emailContent = generateEmailTemplate({
      subject: "🏥 New Major Medical Plan Inquiry",
      body: `
    ...
    await sendMail({
      ...
      subject: "🏥 Major Medical Plan Submission",
      html: emailContent,
    });
    ...
    console.error("Error processing major medical form:", error);
    return new Response(
