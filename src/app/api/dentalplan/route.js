...
export async function POST(req) {
  try {
    ...
    const [result] = await pool.execute(
      `INSERT INTO dentalplan (name, email, message) 
       VALUES (?, ?, ?)`,
      [name, email, message]
    );
    ...
    const emailContent = generateEmailTemplate({
      subject: "🦷 New Dental Plan Submission",
      body: `
        <div style="font-family: Arial, sans-serif; color: #333;">
          <h2>New Dental Plan Request</h2>
    ...
    await sendMail({
      ...
      subject: "🦷 New Dental Plan Form Submission",
      html: emailContent,
    });
    ...
    console.error("Error processing dental plan form:", error);
    return new Response(
